...
public class PsqlRegionQuadtreeIndexer extends BoundingBoxIndexer {

    private static PsqlRegionQuadtreeIndexer instance = null;
    private static boolean isCitus = false;

    // singleton pattern to ensure only one instance existed
    protected PsqlRegionQuadtreeIndexer() {}

    private PsqlRegionQuadtreeIndexer(boolean isCitus) {
        this.isCitus = isCitus;
    }

    // thread-safe instance getter
    public static synchronized PsqlRegionQuadtreeIndexer getInstance(boolean isCitus) {
        if (instance == null) instance = new PsqlRegionQuadtreeIndexer(isCitus);
        return instance;
    }
    ...
    public void createMV(Canvas c, int layerId) throws Exception {
        ...
        // TODO: consider sp-gist
        Statement createIndexStmt = DbConnector.getStmtByDbName(Config.databaseName);
        sql = "CREATE INDEX sp_" + bboxTableName + " ON " + bboxTableName + " USING spgist (geom);";
        System.out.println(sql);
        createIndexStmt.executeUpdate(sql);
        createIndexStmt.close();
